...
# Make sure the last command from each window is written to the history
# file immediately
...
# source executables in $HOME/.bashrc.d (omitting backups)
if [ -d ~/.bashrc.d ] ; then
    for s in $(find ~/.bashrc.d/ -type f | grep -v '~$') ; do
    [ -x "$s" ] && source "$s"
    done
fi
